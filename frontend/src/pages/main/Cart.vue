...
export default {
    ...
    computed: {
        ...
        nameAcitve() {
            ...
            return this.carts.find((item) => item._id === this.idActive).product
                .name;
        },
        total() {
    ...
    <div class="cart">
        <div class="cart__wrap">
            ...
            <ul v-if="cartCount" class="cart__list">
                <li v-for="item in carts" class="cart__item">
                    <Row :gutter='16'>
                        <Col :span='12'>
                            <router-link :to="{ name: 'detail', params: { id: item.product._id } }" class="cart__info">
                                <img class="cart__info-thumbnail" :src="item.product.image" alt="" />
                                <span class="cart__info-name">{{ item.product.name }}</span>
                                <span class="cart__info-size">Size: {{ item.size }}</span>
                            </router-link>
                        </Col>
                        <Col :span='4'>
                            <span class="cart__price">{{ item.product.price.toLocaleString("de-DE") }}đ</span>
                        </Col>
                        <Col :span='4'>
                            <div class="cart__quantity">
                                ...
                                <input class="quanlity" type="number" min="1" name="quantity[2]" v-model="item.qty" />
                                <button class="btn-up" @click="item.qty < item.product.stock &&item.qty++">
                                    <i class="fa-solid fa-plus"></i>
                                </button>
                            </div>
                        </Col>
                        <Col :span='4'>
                            <span class="cart__price">{{ (item.product.price * item.qty).toLocaleString("de-DE") }}đ</span>
                        </Col>
                ...
                <Modal title="Warning" :visible="visible" @ok="deleteCart" @cancel="closeModal" :destroyOnClose='true'>
                        Do you want to remove <span style="font-weight: 700;">{{ nameAcitve}}</span> from your cart?
                </Modal>
            </ul>
        ...
        <div class="cart__bottom">
            <div class="cart__total">Total({{cartCount}} item): <span>{{ total.toLocaleString("de-DE") }}đ</span></div>
            <button class="cart__buy" :class="{disabled: !cartCount}" :disabled="!cartCount" @click="toCheckout">Check out</button>
        </div>
    </div>
</template>
