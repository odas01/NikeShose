...
export default {
    ...
    methods: {
            ...
            if (value) {
                try {
                    ...
                    this.categorys.forEach((category) => {
                        category.thumbnail =
                            "data:image/png;base64," + category.thumbnail;
                    });
                } catch (e) {
                    console.log(e);
                }
        ...
        getThumbnailsType(type) {
            switch (type) {
                case "men":
                    return "/src/assets/image/category/men.png";
                case "women":
                    return "/src/assets/image/category/women.png";
                default:
                    return "/src/assets/image/category/kid.png";
            }
        },
...
<div class="db__content-header">
    <input class="db__content-search" type="text" :disabled='isFilter' placeholder="Search by category name" @keyup.enter="seacrhCategorys" />
    <select name="type" id="" @change='setFilter($event, "category")' :disabled='isSearch'>
    ...
    <Drawer title="Thêm danh mục" placement="right" :closable="false" :visible="visible" @close="closeDrawer" width="50%">
        <form method="post" class="db__add" @submit="submitForm" ref="form">
            ...
            <div>
                ...
                <input type="file" accept="image/*" name="thumbnail" ref="thumbnail" />
            </div>
            <div class="checkbox d-flex align-items-center mb-4" ref="types">
                ...
                <div class="d-flex">
                    <div class="d-flex align-items-center me-5">
                        <span>Men</span>
                        <input type="checkbox" value="men" name="parent" @click="handleCheckbox" />
                    </div>
                    <div class="d-flex align-items-center me-5">
                        <span>Women</span>
                        <input type="checkbox" value="women" name="parent" @click="handleCheckbox" />
                    </div>
                    <div class="d-flex align-items-center me-5">
                        <span>Kid</span>
                        <input type="checkbox" value="kid" name="parent" @click="handleCheckbox" />
                    </div>
                </div>
            </div>
        ...
        <tbody>
            <tr v-for="category, index in categorys">
                ...
                <td class="d-flex justify-content-center">
                    <span>
                        <img v-for="item in category.types" :src="getThumbnailsType(item)" alt="" class="me-3" />
                    </span>
                </td>
                <td class="text-center">
                    <span class="px-1 py-1" @click="changeStatus(category)">
                        <svg v-if="category.open" stroke="currentColor" style="color: #0e9f6e;" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" class="text-green-500" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                ...
                <td class="">
                    <div>
                        <i class="fa-solid fa-pen-to-square" data-bs-toggle="tooltip" data-bs-placement="top" title="Chỉnh sửa" @click="showEidit(category)"> </i>
                        <i class="fa-regular fa-trash-can ms-3" data-bs-toggle="tooltip" data-bs-placement="top" title="Xóa" @click="deleteCategory(category._id, $event)"> </i>
                    </div>
                </td>
            </tr>
        </tbody>
